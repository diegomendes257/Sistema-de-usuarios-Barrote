...
//------------------------ADICIONA JOGO--------------------------------------------------
		...
		public function criaJogo($nomeJogo, $valorJogo, $premio, $inicioJogo, $fimJogo, $descricao, $path_capa, $ativado){
			global $conexao;
			...
			if($consulta->rowCount() > 0){
				...
				$cadastroJogo = "INSERT INTO jogos_aposta(nome_jogo, valor_jogo, premio_jogo, inicio_jogo, fim_jogo, descricao, path_capa, ativado) VALUE(:nome_jogo, :valor_jogo, :premio_jogo, :inicio_jogo, :fim_jogo, :descricao, :path_capa, :ativado)";
...
//------------------------CONSULTA JOGO--------------------------------------------------
		...
		public function exibeJogo(){
			...
			$exibe = 'SELECT nome_jogo, valor_jogo, inicio_jogo, path_capa, ativado FROM jogos_aposta';
			...
			while($exibeTudo = $exibe->fetch(PDO::FETCH_ASSOC)){
				...
				echo "<div class='col-6 col-md-4'>
						<div class='d-flex p-2 align-items-center flex-column border rounded'>
							<img class='fotoPerfil mb-3 w-100' src='./adm/".$exibeTudo['path_capa']."' alt='Capa ".$exibeTudo['nome_jogo']."'>
							<h5>".$exibeTudo['nome_jogo']."</h5>
							<p class='text-center'>
								Inscrição: Barro ".number_format($exibeTudo['valor_jogo'] , 2, ',',' . ')."<br />
								".date_format($dataCriadoJogo, 'd/m/Y').' às '.date_format($dataCriadoJogo, 'H:i')."
							</p>
							<button onclick='window.location='".$exibeTudo['path_capa']."' class='btn btn-primary w-100 btn-sm text-truncate'>PARTICIPAR</button>
						</div>
					</div>
				";				
			}
